...
DeclAstNode* Parser_declaration(Parser* parser) {  // @TODO
  ...
  // has been provided.
  if(!decl->identifier) {
    Error_report_error(PARSER, peek()->line_number, "Missing identifier in declaration");
    THROW_ERROR(parser);
  }
...
static CType* declaration_specifiers(Parser* parser) {  // @TODO
  ...
  if (err) {
    Error_report_error(PARSER, peek()->line_number, err);
    THROW_ERROR(parser);
  }
  return type;
}
...
CType* Parser_type_name(Parser* parser) {  // @TODO
  ...
  if(decl->decl_type == CONCRETE) {
    Error_report_error(PARSER, peek()->line_number, "Type names must not have an identifier");
    THROW_ERROR(parser);
  }
  return decl->type;
}
