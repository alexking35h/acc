...
static void function_end(IrFunction * function)
{
    for(int i = 0;i < function->register_count;i++)
    {
        for(IrBasicBlock * bb = function->head;bb;bb=bb->next)
        {
            ...
            if(!register_set_test(bb->entry_set, i)) continue;
            if(!register_set_test(bb->exit_set, i)) continue;

            int bb_start = bb->head->pos;
            int bb_finish = bb->tail->pos;

            function->registers[i].liveness.start = MIN(
                function->registers[i].liveness.start, bb_start
            );
            function->registers[i].liveness.finish = MAX(
                function->registers[i].liveness.finish, bb_finish
            );
        }
    }
}
