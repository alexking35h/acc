...
static void panic_mode_declaration(void** state) {
  ...
  src = "int a = 3 + / 3;int b;";
  ast = "(D [signed int], b)";
  expect_report_error(PARSER, 1, "Expected expression, got '/'");
  assert_true(test_ast_compare_decl(src, ast));

  expect_report_error(PARSER, 1, "Expecting 'constant', got 'identifier'");
  assert_true(test_ast_compare_decl("int a[b];", ""));

  // Declarators must have an identifier (6.7.6)
  expect_report_error(PARSER, 1, "Missing identifier in declaration");
...
static void panic_mode_parameter_list(void** state) {
  ...
  expect_report_error(PARSER, 1, "Invalid type");
  assert_true(test_ast_compare_decl(src, ast));
...
static void panic_mode_compound_statement(void** state) {
  ...
  expect_report_error(PARSER, 2, "Expecting ';', got 'identifier'");
  assert_true(test_ast_compare_decl(src, ast));
  ...
  src = "void p(){\n  a=1\n}";
  ast = "(D [f() [void]], p, {B })";
  expect_report_error(PARSER, 3, "Expecting ';', got '}'");
  assert_true(test_ast_compare_decl(src, ast));
}
...
static void type_error(void** state) {
  ...
  expect_report_error(PARSER, 1, "Functions cannot return functions (try Python?)");
  assert_true(test_ast_compare_decl("int a()();", ""));
  expect_report_error(PARSER, 1, "Functions cannot return arrays (try Python?)");
  ...
  expect_report_error(PARSER, 1, "Type names must not have an identifier");
