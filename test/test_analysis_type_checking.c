...
static void postfix_operators(void** state) {
    ...
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_ptr[_int]"), test_symbol_table);

    // 6.5.2.1 (2): Successive postscript operators designate an element of a 
    // multidimensional array object.
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_ptr_ptr[_int][_int]"), test_symbol_table);

    // 6.5.2.2  (2): (In a function call) the number of arguments shall agree with the number
    // of parameters. 
    expect_report_error(ANALYSIS, 1, 1, "Invalid number of arguments to function. Expected 1, got 2");
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr(" _function(_int, _int)"), test_symbol_table);
    ...
    // assigned to an object with the unqualified version of the type of its corresponding parameter.
     ...
     analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_function(_ptr)"), test_symbol_table);

    // Error check - cannot call something that is not a function.
    expect_report_error(ANALYSIS, 1, 0, "Not a function");
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int(2,3)"), test_symbol_table);
}
...
static void unary_operators(void** state) {
    ...
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("*_ptr = 1"), test_symbol_table);
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("**(&_ptr) = 1"), test_symbol_table);
...
static void arithmetic_operators(void** state) {
    ...
    // type.
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int * _char"), test_symbol_table);
    expect_report_error(ANALYSIS, 1, 5,"Invalid operand type to binary operator '*'");
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int * _ptr"), test_symbol_table);
    expect_report_error(ANALYSIS, 1, 5,"Invalid operand type to binary operator '/'");
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int / _ptr"), test_symbol_table);
    expect_report_error(ANALYSIS, 1, 5,"Invalid operand type to binary operator '%'");
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int % _ptr"), test_symbol_table);
    ...
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int + _ptr_ptr"), test_symbol_table);
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int + _ptr"), test_symbol_table);
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int + _char"), test_symbol_table);
    expect_report_error(ANALYSIS, 1, 5,"Invalid operand type to binary operator '+'");
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_ptr + _ptr"), test_symbol_table);
    ...
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_ptr - _int"), test_symbol_table);
    expect_report_error(ANALYSIS, 1, 5,"Invalid operand type to binary operator '-'");
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_ptr - _ptr_ptr"), test_symbol_table);
    ...
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("\"abc\" << 3"), test_symbol_table);
}
...
static void comparison_operators(void** state) {
    ...
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_ptr >= _ptr"), test_symbol_table);
    expect_report_error(ANALYSIS, 1, 5, "Invalid operand type to binary operator '<'");
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_ptr < _char"), test_symbol_table);

    // 6.5.9 (equality operators) Both operators have arithmetic type, both operands are
    // pointers to compatible types
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int == _char"), test_symbol_table);
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_char != _int"), test_symbol_table);
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_ptr == _ptr"), test_symbol_table);
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_ptr != _ptr"), test_symbol_table);
    expect_report_error(ANALYSIS, 1, 5,"Invalid operand type to binary operator '!='");
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_ptr != _int"), test_symbol_table);
    ...
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_function == _ptr"), test_symbol_table);
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("&_function && 98"), test_symbol_table);
}

static void bitwise_operators(void** state) {
    // &, |, ^  6.5.10, 6.5.11, 6.5.12 - operators must be arithmetic.
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int & _char | _long_int ^ _int"), test_symbol_table);
    expect_report_error(ANALYSIS, 1, 5,"Invalid operand type to binary operator '&'");
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int & _ptr"), test_symbol_table);
    expect_report_error(ANALYSIS, 1, 5,"Invalid operand type to binary operator '|'");
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int | _ptr"), test_symbol_table);
    expect_report_error(ANALYSIS, 1, 9,"Invalid operand type to binary operator '^'");
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_ptr_ptr ^ _long_int"), test_symbol_table);
}

static void logical_operators(void** state) {
    // &&, || 6.5.13, 6.5.14 - must be scalar (arithmetic & pointer)
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int && _char || _long_int"), test_symbol_table);
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_ptr && _char || _ptr"), test_symbol_table);
    ...
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_ptr?_ptr:_ptr"), test_symbol_table);

    expect_report_error(ANALYSIS, 1, 1,"Invalid types in tertiary expression");
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("1?_ptr:_int"), test_symbol_table);
}
...
static void assignment_operators(void** state) {
    ...
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int = *_ptr = 1"), test_symbol_table);
    ...
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("1 ? 1 : 1 = 2 + 1 = 3 = 1"), test_symbol_table);
    ...
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int = 1"), test_symbol_table);

    // - The left value has pointer type, and both operators pointers compatible types.
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_int = _int"), test_symbol_table);
    ...
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_ptr = _int = _ptr = _ptr_ptr"), test_symbol_table);
    ...
    analysis_ast_walk_expr(MOCK_ERROR_REPORTER, parse_expr("_ptr = &_function"), test_symbol_table);
}
...
static void declaration_initializer(void** state) {
    ...
    analysis_ast_walk_decl(MOCK_ERROR_REPORTER, parse_decl("int *b = _int;"), test_symbol_table);
}
